...
final class SQLiteRemoteDocumentCache implements RemoteDocumentCache {
    ...
    private Map<DocumentKey, MutableDocument> getAll(List<ResourcePath> collections, FieldIndex.IndexOffset offset, int count) {
        ...
        BackgroundQueue backgroundQueue = new BackgroundQueue();
        Map<DocumentKey, MutableDocument> results = new HashMap<>();
        this.db.query(sql.toString()).binding(bindVars).forEach(new SQLiteRemoteDocumentCache$$ExternalSyntheticLambda1(this, backgroundQueue, results));
        backgroundQueue.drain();
        return results;
    }
    ...
    public void m385lambda$getAll$1$comgooglefirebasefirestorelocalSQLiteRemoteDocumentCache(BackgroundQueue backgroundQueue, Map<DocumentKey, MutableDocument> results, Cursor row) {
        (row.isLast() ? Executors.DIRECT_EXECUTOR : backgroundQueue).execute(new SQLiteRemoteDocumentCache$$ExternalSyntheticLambda0(this, row.getBlob(0), row.getInt(1), row.getInt(2), results));
    }
    ...
    public /* synthetic */ void m386lambda$processRowInBackground$2$comgooglefirebasefirestorelocalSQLiteRemoteDocumentCache(byte[] rawDocument, int readTimeSeconds, int readTimeNanos, Map results) {
