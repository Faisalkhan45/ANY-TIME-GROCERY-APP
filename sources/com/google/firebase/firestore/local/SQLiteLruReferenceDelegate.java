...
class SQLiteLruReferenceDelegate implements ReferenceDelegate, LruDelegate {
    ...
    public int removeOrphanedDocuments(long upperBound) {
        ...
        while (true) {
            boolean z = false;
            if (resultsRemaining) {
                if (this.persistence.query("select path from target_documents group by path having COUNT(*) = 1 AND target_id = 0 AND sequence_number <= ? LIMIT ?").binding(Long.valueOf(upperBound), 100).forEach(new SQLiteLruReferenceDelegate$$ExternalSyntheticLambda2(this, count, docsToRemove)) == 100) {
                    z = true;
                }
                resultsRemaining = z;
            } else {
    ...
    public /* synthetic */ void m374lambda$removeOrphanedDocuments$2$comgooglefirebasefirestorelocalSQLiteLruReferenceDelegate(int[] count, List docsToRemove, Cursor row) {
