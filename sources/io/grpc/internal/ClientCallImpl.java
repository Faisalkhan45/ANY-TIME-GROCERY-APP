...
final class ClientCallImpl<ReqT, RespT> extends ClientCall<ReqT, RespT> {
    ...
    private static void logIfContextNarrowedTimeout(Deadline effectiveDeadline, @Nullable Deadline outerCallDeadline, @Nullable Deadline callDeadline) {
        ...
        if (logger.isLoggable(Level.FINE) && effectiveDeadline != null && effectiveDeadline.equals(outerCallDeadline)) {
            long effectiveTimeout = Math.max(0, effectiveDeadline.timeRemaining(TimeUnit.NANOSECONDS));
            StringBuilder builder = new StringBuilder(String.format(Locale.US, "Call timeout set to '%d' ns, due to context deadline.", new Object[]{Long.valueOf(effectiveTimeout)}));
            if (callDeadline == null) {
                builder.append(" Explicit call timeout was not set.");
            } else {
                long callTimeout = callDeadline.timeRemaining(TimeUnit.NANOSECONDS);
                builder.append(String.format(Locale.US, " Explicit call timeout was '%d' ns.", new Object[]{Long.valueOf(callTimeout)}));
            }
